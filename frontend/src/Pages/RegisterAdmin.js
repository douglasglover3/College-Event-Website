...
export default function RegisterAdmin({setUser}) {
    ...
    {
        if(validateInputs())
            Axios.post(API_URL + "/users/createAdmin", {
                userID: userID,
    ...
    return (
        <div>
            <Section color="white">
                <div style={{width:"70%"}}>
                    <h4 style={{marginBottom:"30px"}}>Register as an Admin</h4>
                        <Form.Group className="mb-2">
                            <Form.Label>University Name</Form.Label>
                            <Form.Control type="text" placeholder='University Name' onChange = {(input) =>{setUniversity(input.target.value)}}/>
                        </Form.Group>
                        <Form.Group className="mb-2">
                            <Form.Label>RSO Names</Form.Label>
                            <Form.Control type="text" placeholder='Enter RSO Name' onChange = {(input) =>{setRsos(input.target.value)}}/>
                        </Form.Group>
                        <Form.Group className="mb-2">
                        ...
                        <Button className="regular" type="button" onClick={() => registerAdmin()}>
                            Register
                        </Button>
                </div>
            </Section>
